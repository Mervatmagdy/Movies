...
static Future<DiscoverResponse> getRecommend()async{
  ...
  static Future<Genres?> getMovieDetailsById({required num? id}) async {
    // https://newsapi.org/v2/everything?q=apple&from=2023-10-21&to=2023-10-21&sortBy=popularity&apiKe
    Uri url = Uri.https(ApiConstant.baseUrl, ApiConstant.detailsUrl, {
      "api_key": "001980e37e125ade2ff7f7ff71d9e93a",
      'movie_id': id
    });
    try {
  ...
  static Future<SamilerResponse?> getSimilarMovies({required String? id}) async {
    // https://newsapi.org/v2/everything?q=apple&from=2023-10-21&to=2023-10-21&sortBy=popularity&apiKe
